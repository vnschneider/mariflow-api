...
export class WhatsAppService extends EventEmitter {
  ...
  private initializeClient(): void {
    try {
      ...
      throw new WhatsAppError(
        "Falha ao inicializar cliente WhatsApp",
        "INIT_ERROR"
      );
    }
  }
  ...
  private setupEventListeners(): void {
    ...
    this.client.on("ready", () => {
      ...
      this.emit("ready");
      logger.info(
        `WhatsApp pronto! Número: ${this.phoneNumber}, Nome: ${this.name}`
      );
    });
  ...
  public async createGroup(
    ...
    try {
      ...
      this.lastSeen = Date.now();
      logger.info(
        `Grupo criado: ${name} com ${participants.length} participantes`
      );
      return result as unknown as GroupChat;
    } catch (error) {
  ...
  public async getGroupInviteLink(groupId: string): Promise<string> {
    ...
    try {
      ...
      throw new WhatsAppError(
        "Falha ao obter link de convite",
        "GET_INVITE_ERROR"
      );
    }
  }
  ...
  public async unblockContact(contactId: string): Promise<void> {
    ...
    try {
      ...
      throw new WhatsAppError(
        "Falha ao desbloquear contato",
        "UNBLOCK_CONTACT_ERROR"
      );
    }
  }
  ...
  public async getMessages(
    ...
    try {
      ...
      logger.debug(
        `Retornando ${messages.length} mensagens do chat: ${chatId}`
      );
      return messages;
    } catch (error) {
  ...
  public async getClientInfo(): Promise<any> {
    ...
    try {
      ...
      throw new WhatsAppError(
        "Falha ao obter informações do cliente",
  ...
  public async restart(): Promise<void> {
    try {
      ...
      throw new WhatsAppError(
        "Falha ao reiniciar cliente WhatsApp",
        "RESTART_ERROR"
      );
    }
  }
